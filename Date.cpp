...
bool checkDate(std::string date) {
	std::vector<std::string> tokens2 = split(date, '/');
	std::vector<std::string> tokens1 = split(tokens2[0], ':');
	if (tokens1.size() != 2 && tokens2.size() != 4) return false;
	try {
		int year = std::stoi(tokens2[3]);
		if (year < 1900 || year > 2017) return false;
		
		int month = std::stoi(tokens2[2]);
		if (month < 1 || month > 12) return false;

		int day = std::stoi(tokens2[1]);
		if (day < 1 || day > Date::max_day_number(year, month)) return false;

		int hour = std::stoi(tokens1[1]); 
		if (hour < 0 || hour > 23) return false;

		int min = std::stoi(tokens1[0]);
...
Date::Date(int min, int hour, int day, int month, int year) {
	...
	if ((hour < 0) || (hour > 23)) {
		throw std::out_of_range("Incorrect hour (0..23)!");
	}
	this->hour = hour;

	if ((min < 0) || (min > 59)) {
		throw std::out_of_range("Incorrect minutes (0..59)!");
	}
	this->min = min;
}

std::string const Date::to_string() {
	return   std::to_string(min) + ':' + std::to_string(hour) + 
		 '/' + std::to_string(day) + '/' + std::to_string(month) + '/' + std::to_string(year);
}

std::string static to_string(Date date) {
	return   std::to_string(date.min) + ':' + std::to_string(date.hour) +
...
bool Date::StrToDate(std::string str, Date & date) {
	...
	std::vector<std::string> tokens1 = split(str, '/');
	std::vector<std::string> tokens2 = split(tokens1[0], ':');

	try {
		date.min = std::stoi(tokens2[0]);
	}
